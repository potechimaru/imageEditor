...
import Img2ImgPromptInput from '../DrawingComponent/Img2imgPromptInput';
  ...
  return (
    <div>
      <div className="flex justify-center m-3">
        ...
        <p className="text-xl -translate-y-0.5">画像を変換</p>
      ...
      <div className="flex flex-col items-center justify-center gap-y-10 my-10">
        {!originalUrl ? (
          <Image
            ...
            width={150}
            height={405}
            onClick={() => setIsDisplayOriginalSelecter(true)}
          />
        ) : (
          <Image
            src={originalUrl}
      ...
      {isDisplayOriginalSelecter && (
        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-lg border z-50 w-96 p-6 space-y-4">
          ...
          <button
            ...
            onClick={() => {
              setIsDisplayOriginalSelecter(false);
              setIsShowExportedImage(true);
            }}
          >
            エクスポート画像から選択
          </button>
          <button
            onClick={() => setIsDisplayOriginalSelecter(false)}
      ...
      {isShowImportedImage && previewUrls && (
        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-lg border z-50 w-96 p-6 space-y-4 overflow-y-auto max-h-[80vh]">
          <h2 className="text-xl font-medium text-center mb-4">元画像を選択</h2>
          ...
          <button
            onClick={() => setIsShowImportedImage(false)}
      ...
      {isShowExportedImage && exportedUrls && (
        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-lg border z-50 w-96 p-6 space-y-4 overflow-y-auto max-h-[80vh]">
          <h2 className="text-xl font-medium text-center mb-4">元画像を選択</h2>
          <div className="grid grid-cols-3 gap-2">
          ...
          <button
            onClick={() => setIsShowExportedImage(false)}
            className="mt-2 px-4 py-1 bg-blue-600 text-white"
          >
            閉じる
          </button>
...
export default MenuImg2ImgSection;
